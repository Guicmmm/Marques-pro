...
# ============================================================
# MARQUES PRO V18
# Backend de análise esportiva MULTIFONTE
#
# FONTES:
#   1. SofaScore
#   2. ESPN
#   3. API-Football
#   4. Flashscore (camada complementar preparada)
#
# IMPORTANTE:
# O sistema NÃO depende da API-Football para encontrar jogos.
# ============================================================
PORT = int(
    os.getenv(
        "PORT",
...
# Opcional.
# Se no futuro você contratar uma API/serviço compatível
# com Flashscore, pode informar a URL aqui.
#
# Não usamos endpoints internos não documentados do Flashscore
# como dependência obrigatória.
FLASHSCORE_API_URL = os.getenv(
    "FLASHSCORE_API_URL",
    ""
).strip()
CACHE_TTL = int(
...
# LIGAS
#
# sofa_id = ID do torneio no SofaScore.
#
# Os IDs são independentes dos IDs da API-Football.
# ============================================================
LEAGUES = {
    "Brasil": {
        "id": 71,
...
#
# Podemos adicionar estas competições ao seletor sem depender
# da API-Football.
# ============================================================
INTERNATIONAL_LEAGUES = {
    ...
    "Libertadores": {
        ...
        "odds": "soccer_conmebol_libertadores",
        "sofa_id": 384
    },
    "Sul-Americana": {
        "id": 11,
        ...
        "odds": "soccer_conmebol_sudamericana",
        "sofa_id": 480
    }
}
# Junta ligas nacionais + internacionais.
LEAGUES.update(
    INTERNATIONAL_LEAGUES
)
# ============================================================
...
# NOVA ESTRATÉGIA:
#
# 1. Busca diretamente os eventos do torneio:
#
# /api/v1/unique-tournament/{id}/events/next/{page}
#
# 2. Se necessário, consulta scheduled-events.
#
# Isso evita o problema do V17 em que o nome "Brasileirão
# Série A" não coincidia com "Brasileirão Betano".
# ============================================================
def sofascore_tournament_events(
    sofa_id,
    direction="next",
    pages=3
):
    ...
    for page in range(
        ...
        # Se a API informar que não existe
        # próxima página, paramos.
        has_next = data.get(
            "hasNextPage"
        )
...
def sofascore_fixture(
    ...
    try:
        ...
        print(
            "[SOFASCORE] "
            f"normalização: {error}"
        )
        return None
def sofa_event_date_in_window(
    event,
    start_date,
    end_date
):
...
def sofascore_fixtures(
    ...
    # ========================================================
    # 1. PRÓXIMOS JOGOS DO TORNEIO
    # ========================================================
    next_events = sofascore_tournament_events(
        sofa_id,
        "next",
        6
    )
    print(
        f"[SOFASCORE] "
        f"next: "
        f"{len(next_events)} eventos"
    )
    for event in next_events:
        if sofa_event_date_in_window(
            event,
            today,
            end
        ):
    ...
    # 2. FALLBACK POR DATA
    #
    # Isso amplia a cobertura quando o endpoint de torneio
    # não devolver algum evento futuro.
    # ========================================================
    if len(events) == 0:
        print(
            "[SOFASCORE] "
            "next não encontrou jogos; "
            "ativando fallback por data"
        )
    for i in range(
        search_days + 1
    ):
        ...
        scheduled = sofascore_scheduled_events(
            date_string
        )
...
# FLASHCORE
#
# O Flashscore não fornece uma API pública simples e estável
# para esse tipo de integração.
#
# Portanto não vamos inventar endpoints internos.
#
# Porém deixamos uma camada preparada para uma API/serviço
# compatível configurado via FLASHSCORE_API_URL.
# ============================================================
def flashscore_fixtures(
    league_name,
    days=7
):
    if not FLASHSCORE_API_URL:
        print(
            "[FLASHSCORE] "
            "Nenhuma API compatível configurada."
        )
        return []
    try:
        league = LEAGUES.get(
...
def normalize_team_name(
    ...
    # através de substituições comuns.
    replacements = {
        "fc": "",
        "afc": "",
        "cf": "",
...
def fixture_identity(
    ...
    if parsed:
        ...
        # Não exigimos minuto exato.
        #
        # Provedores diferentes podem informar:
        # 19:00 / 19:05 / 19:15.
        #
        # Agrupamos por data + equipes.
        date_key = local.strftime(
            "%Y-%m-%d"
        )
    else:
        date_key = ""
...
def get_fixtures(
    ...
    print(
        "[FIXTURES] BUSCA GLOBAL MULTIFONTE V18"
    )
    print(
        f"[FIXTURES] Liga: {league_name}"
    )
    print(
        f"[FIXTURES] Janela: {search_days} dias"
    )
    print(
        f"[FIXTURES] Hoje: {brazil_today()}"
    )
    all_fixtures = []
    source_counts_raw = {}
    # ========================================================
    # 1. SOFASCORE
    #
    # Agora vem primeiro porque é a principal fonte de
    ...
    # É complementar.
    #
    # Se a chave Free bloquear a temporada 2026,
    # não interrompe a busca.
    # ========================================================
    try:
    ...
    # 4. FLASHSCORE
    #
    # Só executa se FLASHSCORE_API_URL estiver configurada.
    # ========================================================
    try:
        flash_data = flashscore_fixtures(
            league_name,
            search_days
        )
        source_counts_raw[
    ...
    print(
        "[FIXTURES] "
        f"Brutos: {len(all_fixtures)}"
    )
    print(
        "[FIXTURES] "
        f"Fontes brutas: {source_counts_raw}"
    )
    print(
        "[FIXTURES] "
...
if __name__ == "__main__":
    ...
    print(
        "Fixtures: "
        "SofaScore + ESPN + API-Football + Flashscore"
    )
    print(
        f"Ligas disponíveis: "
        f"{len(LEAGUES)}"
