...
def build_model(fixture):
    ...
    # Quanto mais jogos reais temos, maior o peso do histórico.
    sample = min(hp["games"], ap["games"])
    data_weight = clamp(0.35 + sample * 0.045, 0.35, 0.78)

    # Ataque próprio + defesa adversária + média da liga.
    home_attack = weighted_blend(hp["gf"], lgf, data_weight)
    away_defense = weighted_blend(ap["ga"], lga, data_weight)
    away_attack = weighted_blend(ap["gf"], lga, data_weight)
    home_defense = weighted_blend(hp["ga"], lgf, data_weight)

    h_xg = 0.42 * home_attack + 0.38 * away_defense + 0.20 * lgf + 0.16
    a_xg = 0.42 * away_attack + 0.38 * home_defense + 0.20 * lga

    # Forma recente dá um pequeno ajuste, sem deixar um time "invencível".
    form_edge = (hp["wins"] - hp["losses"]) - (ap["wins"] - ap["losses"])
    h_xg *= clamp(1 + form_edge * 0.10, 0.88, 1.14)
    a_xg *= clamp(1 - form_edge * 0.07, 0.88, 1.14)

    h_xg = clamp(h_xg, 0.25, 3.40)
    a_xg = clamp(a_xg, 0.20, 3.00)
    total_xg = h_xg + a_xg

    # Resultado por distribuição de Poisson.
    hw = dr = aw = 0
    for h in range(9):
        for a in range(9):
    ...
    # Estatísticas: histórico das equipes + baseline da liga.
    corners = clamp(weighted_blend(hp["corners"] + ap["corners"], lc * 2, data_weight), 5.0, 15.5)
    cards = clamp(weighted_blend(hp["cards"] + ap["cards"], lcards * 2, data_weight), 1.0, 10.0)
    shots = clamp(weighted_blend(hp["shots"] + ap["shots"], ltotalshots, data_weight), 12.0, 40.0)
    sot = clamp(weighted_blend(hp["sot"] + ap["sot"], lsot, data_weight), 2.0, 16.0)

    # Mistura a taxa real de BTTS com o modelo de gols.
    btts_model = poisson_btts(h_xg, a_xg)
    btts = clamp(0.68 * btts_model + 0.32 * ((hp["btts_rate"] + ap["btts_rate"]) / 2), 0.03, 0.97)
    ...
    home_sot = clamp(hp["sot"] * 0.60 + sot * 0.40 * (h_xg / max(total_xg, .1)), 1.0, 8.0)
    away_sot = clamp(ap["sot"] * 0.60 + sot * 0.40 * (a_xg / max(total_xg, .1)), 1.0, 8.0)
    ...
    # Confiança é qualidade da evidência, não garantia de acerto.
    evidence = clamp(45 + sample * 4 + data_weight * 20, 48, 86)
    top_prob = max(hw, dr, aw, btts, poisson_over(total_xg, 1.5))
    confidence = round(clamp(evidence * 0.65 + top_prob * 100 * 0.35, 50, 91), 1)
    ...
    for m in markets:
        p = m["probability"]
        # Evita recomendar mercados extremamente específicos só por terem probabilidade alta.
        if p >= 72:
            best.append({
                **m,
    ...
    return {
        ...
        "metrics": {
            ...
            "model_type": "forma real + Poisson + baseline da liga"
        },
        "best_bets": best,
