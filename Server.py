...
def af(endpoint, params=None, ttl=CACHE_TTL):
    if not AF_KEY:
        return None
...
def poisson_btts(home_lam, away_lam):
    home_no = math.exp(-home_lam)
    away_no = math.exp(-away_lam)

    return 1 - home_no - away_no + (
        home_no * away_no
    )
...
def espn_scoreboard(league, date_value):
    if not league:
        return None
...
def espn_summary(league, event_id):
...
def espn_fixture(event, league_name):
    ...
    return {
        "fixture": {
            ...
            "date": event.get("date"),
            "timestamp": int(
                iso_date(event.get("date")).timestamp()
            )
            if iso_date(event.get("date"))
            else None,
            "status": {
        ...
        "goals": {
            "home": parse_num(home.get("score"), 0),
            "away": parse_num(away.get("score"), 0)
        },
    ...
    while current <= today:
        ...
        if data:
            ...
            for event in data.get(
                ...
                home_id = str(
                    fixture["teams"]["home"].get("id")
                )

                away_id = str(
                    fixture["teams"]["away"].get("id")
                )
...
def recent_profile(
    ...
    return {
        ...
        "shots": round(
            sum(shots) / len(shots),
            2
        ) if shots else default["shots"],

        "sot": round(
            sum(sot) / len(sot),
            2
        ) if sot else default["sot"],

        "corners": round(
            sum(corners) / len(corners),
            2
        ) if corners else default["corners"],

        "cards": round(
            sum(cards) / len(cards),
            2
        ) if cards else default["cards"]
    }


# ============================================================
# DESFALQUES API FOOTBALL
# ============================================================
...
def squad_context(
    ...
    for team_id, team in context["teams"].items():

        team["injury_count"] = 0
        team["suspension_count"] = 0
        team["impact_score"] = 0
    ...
    for injury in context.get(
        ...
        if team_id not in context["teams"]:
            context["teams"][team_id] = {
                "id": team_id,
                "name": injury.get("team"),
                "formation": None,
                "starting_xi": [],
                "substitutes": [],
                "injury_count": 0,
...
def build_model(
    ...
    if home_context:
        for team in home_context.get(
            ...
            if str(team.get("id")) == str(home_id):
    ...
    if away_context:
        for team in away_context.get(
            ...
            if str(team.get("id")) == str(away_id):
                away_impact = team.get(
                    "impact_score",
                    0
                )

    # impacto moderado para não deixar uma
    # informação incompleta destruir o modelo
    ...
    add_market(
        "home_shots_on_target_over_2_5",
        f"{home.get('name')} +2.5 chutes no gol",
        poisson_over(
            home_profile["sot"],
            2.5
        )
    ...
    add_market(
        "away_shots_on_target_over_2_5",
        f"{away.get('name')} +2.5 chutes no gol",
        poisson_over(
            away_profile["sot"],
            2.5
        )
...
def event_matches(
    ...
    if (
        home in event_home
        or event_home in home
    ) and (
        away in event_away
        or event_away in away
    ):
        return True
...
def enrich_value(
    ...
    for odd in real_odds:
        ...
        if odd.get("market") == "h2h":

            for key, model in model_map.items():

                if (
                    "home" in key
                    and "vencer" in model["label"].lower()
                ):
                    pass
...
if __name__ == "__main__":
    ...
    print(
        f"Marques Pro iniciado na porta {PORT}"
    )

    print(
        f"API-Football: "
        f"{'ATIVA' if AF_KEY else 'INATIVA'}"
    )

    print(
        f"Odds API: "
        f"{'ATIVA' if ODDS_KEY else 'INATIVA'}"
    )
